...
using TTSI = TestDataFactoryGenerator.TestData.Served.IOrderService;
using TTSIG = TestDataFactoryGenerator.TestData.Served.IOrderService.GetOrdersResquest;
...
/// - <see cref="TestDataFactoryGenerator.TestData.Served.IOrderService+GetOrdersResponse"/><br/>
/// - <see cref="TestDataFactoryGenerator.TestData.Served.IOrderService+GetOrdersResquest"/><br/>
...
internal partial class Tdf
{
    ...
    public TTSI.GetOrdersResponse GenerateGetOrdersResponse(
        ImmutableArray<TTS1.Order>? orders = null)
    {
        return new TTSI.GetOrdersResponse(
            Orders: orders ?? [..GetSome(() => GenerateOrder())]);
    }

    public TTSI.GetOrdersResquest GenerateGetOrdersResquest(
        DateTimeOffset? from = null,
        DateTimeOffset? to = null,
        TTSIG.OrderTypeFilter? orderType = null)
    {
        return new TTSI.GetOrdersResquest(
            from: from ?? random.NextDateTimeOffset(),
            to: to ?? random.NextDateTimeOffset(),
            OrderType: orderType ?? random.NextEnum<TTSIG.OrderTypeFilter>());
    }
    ...
    public TTS1.Order GenerateOrder(
        ...
        OptionalRef<TTS1.Delivery> delivery = default,
        ...
        TTS1.AllCollections? allCollections = null)
    {
        return new TTS1.Order(
            ...
            Delivery: delivery.Unwrap(whenAutoGenerated: () => GenerateDelivery()),
...
internal static class RandomExtensions
{
    ...
    public static T NextEnum<T>(this Random r) where T : struct, Enum => Enum.GetValues<T>().OrderBy(_ => r.Next()).First();
}